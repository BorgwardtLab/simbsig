...
# Here are tests for the RadiusNeighborsRegressor implementation.

class Test_RadiusNeighborsRegressor_concept(unittest.TestCase):

    def test_RadiusNeighborsRegressor_concept(self):
        ...
        nn_1 = RadiusNeighborsRegressor(radius=2)
        nn_1.fit(X_s1, y_s1)
        # Test Regression
        self.assertTrue(np.equal(np.array(1), nn_1.predict([[2]])))

        # Concept for predict, 4 training points of 1 dimension, y.shape = (4,)
        nn_2 = RadiusNeighborsRegressor(radius=2) # 3 neighbors for each point
        nn_2.fit(X_s2, y_s2)
        # Test Regression
        self.assertTrue(np.allclose(np.array([2, 3]), nn_2.predict([[0.9], [2.1]])))

        # Concept for predict, 4 training points of 1 dimension, y.shape = (4,1)
        nn_2 = RadiusNeighborsRegressor(radius=2) # 3 neighbors for each point
        nn_2.fit(X_s2, y_s3)
        # Test Regression
        self.assertTrue(np.allclose(np.array([[2], [3]]), nn_2.predict([[0.9], [2.1]])))

class Test_RadiusNeighborsRegressor_sklearn_simbsig(unittest.TestCase):

    def test_RadiusNeighborsRegressor_sklearn_simbsig(self):
        '''Compare simbsig.neighbors.RadiusNeighborsRegressor vs sklearn.neighbors.RadiusNeighborsRegressor
        as gold standard'''
        ...
        for radius in [0.01, 0.1, 1, 2, 3]:
            ...
            if n_dim == 1:
                ...
                metric_lys = ["minkowski", "mahalanobis", "euclidean",  "manhattan", "cosine"]
                        ...
                        for mode in ['arrays', 'hdf5']:
                            for batch_size in [30, INF]:
                                for sample_weights in ['uniform', 'distance']:
                                    ...
                                    inf_string = f'radius: {radius} sample_weights: {sample_weights} metric: {metric} ' \
                                    ...
                                    if metric == "mahalanobis":
                                        nn_sk = RadiusNeighborsRegressor_sk(radius=radius, weights=sample_weights,
                                                                            metric=metric, metric_params={'VI': VI})
                                        nn_bs = RadiusNeighborsRegressor(radius=radius, weights=sample_weights,
                                                                         metric=metric, feature_weights=feature_weights,
                                    ...
                                    ####################################################################################
                                    # fit RadiusNeighborsRegressor objects
                                    nn_sk.fit(X_train_sklearn, y_train)

                                    if mode == 'arrays':
                                        nn_bs.fit(X_train, y_train)
                                    elif mode == 'hdf5':
                                        nn_bs.fit(X_train_used)
                                    ...
                                    with warnings.catch_warnings():
                                        ...
                                        y_pred_bs = nn_bs.predict(X_query_used)

                                    # Have tolerance for different prediction compared to sklearn due to
                                    # small, numerically caused distance differences
                                    acceptance_level = 0.00001
                                    different_preds = np.sum(abs(y_pred_sk-y_pred_bs) > acceptance_level)
                                    same_preds = np.sum(abs(y_pred_sk-y_pred_bs) < acceptance_level)

                                    # The assertTrue statement is executed if there is a match (passed),
                                    # or more than 5% of all predicted targets do not match
                                    if different_preds == 0 or different_preds > n_queries*n_regressions/20:
                                        self.assertTrue(np.allclose(y_pred_sk, y_pred_bs, rtol=1e-3, equal_nan=True),
                                                        msg=inf_string)
                                    else:
                                        print(f'{inf_string} produced {different_preds} predictions with difference >'
                                              f' {acceptance_level}, (while {same_preds} probs match'
                                              f' below {acceptance_level} difference)')
